...
tape( 'the command-line interface prints out the duration corresponding to a millisecond value', opts, function test( t ) {
	var cmd;
	var str;
	...
	cmd = [
		EXEC_PATH,
		'-e',
		'"process.stdin.isTTY = true; process.argv[ 2 ] = \'1000\'; require( \''+fpath+'\' );"'
	];
	...
	function done( error, stdout, stderr ) {
		if ( error ) {
			...
			str = stdout.toString();
			t.strictEqual( str.length > 0, true, 'expected output length' );
			t.strictEqual( str, '1s\n', 'expected value' );
...
tape( 'the command-line interface supports use as a standard stream (delimiter option)', opts, function test( t ) {
	var cmd;
	var str;

	cmd = [
		'printf "1000,2000,3000,4000"',
		'|',
		EXEC_PATH,
		fpath,
		'--split ,'
